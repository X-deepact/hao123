...
func (s *Server) getAllCommonSiteItems(ctx *gin.Context) {

	var req listAllCommonSiteRequest
	...
	commonItems, err := s.store.GetAllCommonSiteItem(ctx, "commonSite", filter, skip, limit)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, errorResponse(err))
	}

	ctx.JSON(http.StatusOK, gin.H{"commonSite": commonItems})
}
