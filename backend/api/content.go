...
func (s *Server) getAllContent(ctx *gin.Context) {
	...
	contentList, err := s.store.GetAllContent(context.Background(), "Content", filter, skip, limit)

	if err != nil {
		ctx.JSON(http.StatusBadRequest, errorResponse(err))
	}

	ctx.JSON(http.StatusOK, gin.H{"contentList": contentList})
}
